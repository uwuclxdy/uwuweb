...
/**
 * Parent Grades View
 * /uwuweb/parent/grades.php
 *
 * Allows parents to view their children's grades in read-only mode
 */

require_once '../includes/db.php';
require_once '../includes/auth.php';
require_once '../includes/functions.php';
require_once 'parent_functions.php';
require_once '../includes/header.php';

// Verify parent role and get parent ID
requireRole(4); // ROLE_PARENT = 4
$parentId = getParentId();
if (!$parentId) {
    echo generateAlert('Starševski račun ni najden.', 'error');
...
$children = getParentStudents($parentId);
if (empty($children)) {
    echo generateAlert('Nimate povezanih otrok v sistemu.');
    include '../includes/footer.php';
    exit;
}

// Get selected student from GET parameter or use the first child
$selectedStudentId = isset($_GET['student_id']) ? (int)$_GET['student_id'] : null;
$selectedStudent = null;

// Verify that parent has access to the selected student
if ($selectedStudentId) {
    if (!parentHasAccessToStudent($selectedStudentId, $parentId)) {
        echo generateAlert('Nimate dostopa do tega učenca.', 'error');
        include '../includes/footer.php';
        exit;
    }

    // Find the selected student details
    foreach ($children as $child) if ($child['student_id'] == $selectedStudentId) {
        $selectedStudent = $child;
        break;
    }
}

// If no student selected or invalid selection, default to first one
if (!$selectedStudent && !empty($children)) {
...
if (empty($studentClasses)) {
    echo generateAlert('Izbrani učenec ni vpisan v noben razred.');
    include '../includes/footer.php';
    exit;
}
...
// Get enrollment ID for this student and class
$stmt = $pdo->prepare("SELECT enroll_id FROM enrollments WHERE student_id = ? AND class_id = ?");
$stmt->execute([$selectedStudentId, $selectedClassId]);
...
renderHeaderCard(
    'Ocene otroka',
    'Pregled ocen in povprečij vašega otroka',
    'parent'
);
?>

<div class="section">
    <!-- Student and Class Selector -->
    <div class="card mb-md">
        <div class="card__content">
            <form method="GET" action="grades.php" class="d-flex flex-wrap justify-between items-center gap-md">
                <div class="d-flex flex-wrap gap-md">
                    <div class="form-group mb-0">
                        <label for="student_selector" class="form-label">Izberite otroka:</label>
                        <select id="student_selector" name="student_id" class="form-select"
                                onchange="this.form.submit()">
                            <?php foreach ($children as $child): ?>
                                <option value="<?= $child['student_id'] ?>"
                                    <?= ($selectedStudentId == $child['student_id']) ? 'selected' : '' ?>>
    ...
    <?php if ($selectedStudent && $selectedClass): ?>
        <!-- Student Info Card -->
        <div class="card mb-md">
            <div class="card__title">
                Učenec: <?= htmlspecialchars($selectedStudent['first_name'] . ' ' . $selectedStudent['last_name']) ?>
            </div>
            <div class="card__content">
                <div class="d-flex flex-wrap justify-between">
                    <div>
                        <p><strong>Razred:</strong> <?= htmlspecialchars($selectedClass['title']) ?></p>
                        <p><strong>Razredna koda:</strong> <?= htmlspecialchars($selectedClass['class_code']) ?></p>
                    </div>
                    <div>
                        <p><strong>Skupno povprečje:</strong></p>
                        <?php
                        $avgClass = '';
                        ...
                        <div class="grade <?= $avgClass ?>"
                             style="font-size: 1.2em; padding: 10px 20px; font-weight: bold;">
                ...
                <!-- Grade Legend -->
                <div class="d-flex gap-md flex-wrap mt-md">
    ...
    <?php else: ?>
        <div class="alert status-warning">
            <div class="alert-content">Izberite otroka in razred za prikaz ocen.</div>
        </div>
    <?php endif; ?>
</div>
