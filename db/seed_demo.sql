...
DELETE
FROM users
WHERE role_id != 1; -- Keep admin users
SET FOREIGN_KEY_CHECKS = 1;

-- Demo Teachers
INSERT INTO users (username, pass_hash, role_id)
VALUES ('janez.novak', '$2y$10$OZIwHNqgCgQCYpZ3iUy9xejJ8Jg9nDjB0C37fRB1UjQT8Jrp8G9.6', 2), -- Password: Teacher123!
...
VALUES ((SELECT user_id FROM users WHERE username = 'janez.novak')),
...
VALUES ('R3A', 'Class R3A', (SELECT teacher_id
                             FROM teachers
                                      JOIN users ON teachers.user_id = users.user_id
                             WHERE username = 'janez.novak')),
       ('R3B', 'Class R3B', (SELECT teacher_id
                             FROM teachers
                                      JOIN users ON teachers.user_id = users.user_id
                             WHERE username = 'maja.kovac'));

-- Demo Students (20 students)
INSERT INTO users (username, pass_hash, role_id)
VALUES ('ana.horvat', '$2y$10$TzKo0xVVyQ81hJvTxL9c1efimpgLxj2Pc.cQv9r1XcL4Qu3yZ2wp2', 3), -- Password: Student123!
...
VALUES ((SELECT user_id FROM users WHERE username = 'ana.horvat'), 'Ana', 'Horvat', '2007-05-12', 'R3A'),
...
-- Demo Parents
INSERT INTO users (username, pass_hash, role_id)
VALUES ('marko.horvat', '$2y$10$2/dNB1GZ5VPSBFJR5vZlLeYEDcVLVlX07bunmCvGT4kjsHdM3CZ8i', 4), -- Password: Parent123!
...
VALUES ((SELECT user_id FROM users WHERE username = 'marko.horvat')),
...
VALUES ((SELECT student_id
         ...
         WHERE username = 'ana.horvat'),
        (SELECT parent_id
         FROM parents
                  JOIN users ON parents.user_id = users.user_id
         WHERE username = 'marko.horvat')),
       ((SELECT student_id
         FROM students
                  JOIN users ON students.user_id = users.user_id
         WHERE username = 'ana.horvat'),
        (SELECT parent_id
         FROM parents
                  JOIN users ON parents.user_id = users.user_id
         WHERE username = 'petra.horvat')),
       ((SELECT student_id
         FROM students
                  JOIN users ON students.user_id = users.user_id
         WHERE username = 'miha.petek'),
        (SELECT parent_id
         FROM parents
                  JOIN users ON parents.user_id = users.user_id
         WHERE username = 'tomaz.petek')),
       ((SELECT student_id
         FROM students
                  JOIN users ON students.user_id = users.user_id
         WHERE username = 'nina.kovac'),
        (SELECT parent_id
         FROM parents
                  JOIN users ON parents.user_id = users.user_id
         WHERE username = 'katja.kovac')),
       ((SELECT student_id
         FROM students
                  JOIN users ON students.user_id = users.user_id
         WHERE username = 'luka.zajc'),
        (SELECT parent_id
         FROM parents
                  JOIN users ON parents.user_id = users.user_id
         WHERE username = 'boris.zajc'));
...
FROM (
         ...
         SELECT '2024-09-19'
         UNION ALL
         SELECT '2024-09-26') AS dates,
     class_subjects cs
WHERE cs.class_id = (SELECT class_id FROM classes WHERE class_code = 'R3A')
...
FROM (
         ...
         SELECT '2024-09-20'
         UNION ALL
         SELECT '2024-09-27') AS dates,
     class_subjects cs
WHERE cs.class_id = (SELECT class_id FROM classes WHERE class_code = 'R3B')
  AND cs.subject_id = (SELECT subject_id FROM subjects WHERE name = 'Matka');
...
SELECT cs.class_subject_id,
       ...
       max_points,
       weight
FROM (SELECT 'Quiz 1' AS item_name, 20.00 AS max_points, 1.00 AS weight
      UNION ALL
      SELECT 'Homework 1', 10.00, 0.50
      UNION ALL
      SELECT 'Midterm Exam', 50.00, 2.00) AS items,
     class_subjects cs
WHERE cs.class_id = (SELECT class_id FROM classes WHERE class_code = 'R3A')
  AND cs.subject_id = (SELECT subject_id FROM subjects WHERE name = 'Matka');
